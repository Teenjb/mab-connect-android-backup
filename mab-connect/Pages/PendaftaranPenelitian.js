...
export default function PendaftaranPenelitian({ navigation }) {
  ...
  const registerPenelitian = async () => {
    console.log(ProposalData);
    const dataForm = new global.FormData()
    dataForm.append('data', JSON.stringify(ProposalData));
    ...
    await client
      .post("/proposal-datas/current", dataForm)
      .then((response) => {
  ...
  return (
    <SafeAreaView className="flex-1 w-screen bg-white">
      ...
      <View className="flex-1 items-center justify-center">
        <View className="mt-20 w-3/4 items-center justify-center">
          <View className=" w-full items-start justify-center mt-2 pt-5">
            ...
            <TextInput className="w-full p-2 m-1 margin-left rounded-full border border-1 border-orange-400"
              ...
              onChangeText={(input) => {
                setProposalData({ ...ProposalData, judul: input });
              }}
            />
            <Text className="ml-4 mt-2 text-blue-600  font-bold">
              Potensi Manfaat
            ...
            <TextInput className="w-full p-2 m-1 margin-left rounded-full border border-1 border-orange-400"
              value={ProposalData.potensimanfaat}
              onChangeText={(input) => {
                setProposalData({ ...ProposalData, potensimanfaat: input });
              }}
            />
            <Text className="ml-4 mt-2 text-blue-600  font-bold">
              Pengajuan Biaya
            ...
            <TextInput className="w-full p-2 m-1 margin-left rounded-full border border-1 border-orange-400"
              value={ProposalData.pengajuanbiaya}
              onChangeText={(input) => {
                setProposalData({ ...ProposalData, pengajuanbiaya: input });
              }}
            />
            <Text className="ml-4 mt-2 text-blue-600  font-bold">
              Nama Penulis
            ...
            <TextInput className="w-full p-2 m-1 margin-left rounded-full border border-1 border-orange-400"
              value={ProposalData.namapenulis}
              onChangeText={(input) => {
                setProposalData({ ...ProposalData, namapenulis: input });
              }}
            />
          </View>
        ...
        <View>
          <Pressable className="w-full px-6 py-2 mx-1 mt-5 items-center justify-center rounded-full bg-blue-600 active:bg-blue-700 font-bol800 shadow-md" onPress={registerPenelitian}>
            <Text
              selectable={false}
